resource "aws_instance" "concourse_web" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "docker run -d --name concourse_web --restart=unless-stopped -v /etc/concourse/keys/:/concourse-keys -p 8080:8080 -p 2222:2222 ${var.conc_image} web --peer-url http://${aws_instance.concourse_web[count.index].private_ip} --postgres-data-source ${var.postgres_connection} --external-url ${var.fqdn} ${var.authentication_config}",
    ]
...
resource "aws_instance" "concourse_worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull ${var.conc_image}",
      "sudo mv ~/keys /etc/concourse/",
      "sudo docker run -d --name concourse_worker --restart=unless-stopped --privileged=true -v /etc/concourse/keys/:/concourse-keys -v /tmp/:/concourse-tmp -p 2222:2222 -p 7777:7777 -p 7788:7788 ${var.conc_image} worker --tsa-host ${aws_elb.concourse_lb.dns_name} --work-dir /concourse-tmp",
    ]
