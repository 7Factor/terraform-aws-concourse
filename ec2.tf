resource "aws_instance" "concourse_web" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker run -d --network host --name concourse_web --restart=unless-stopped -v /etc/concourse/keys/:/concourse-keys ${var.conc_image} web --peer-url http://${self.private_ip}:8080 --postgres-data-source ${var.postgres_connection} --external-url ${var.fqdn} ${var.authentication_config} ${var.web_launch_options}",
    ]
...
resource "aws_instance" "concourse_worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo mv ~/keys /etc/concourse/",
      "sudo find /etc/concourse/keys/ -type f -exec chmod 400 {} \\;",
      "sudo docker run -d --network host --name concourse_worker --privileged=true --restart=unless-stopped -v /etc/concourse/keys/:/concourse-keys -v /tmp/:/concourse-tmp ${var.conc_image} worker --peer-ip ${self.private_ip} --tsa-host ${aws_elb.concourse_lb.dns_name}:2222 --work-dir /concourse-tmp ${var.worker_launch_options}",
    ]
